...
def clean_header_formatting(s):
    if pd.isna(s):
         return ""
    # FIX 1: Instead of splitting and deleting newlines, replace them with a separator
    s_str = re.sub(r'[\r\n]+', ' - ', str(s)).strip()
...
def extract_smart_labels(df):
    """ FIX 2: Dynamically combines leading text columns if statements are split from question numbers. """
    label_cols = []
    for col in df.columns:
        # Convert to numeric to see if it's math data or text data
        test_series = pd.to_numeric(df[col].astype(str).str.replace(r'[,$%]', '', regex=True), errors='coerce')
        # If the column is mostly words (NaN when forced to numbers), it's a label column
        if test_series.isna().mean() > 0.5:
            label_cols.append(col)
        else:
            break # Stop at the first real numeric data column
            
    if not label_cols:
        label_cols = [df.columns[0]] # Fallback to col 1 if it gets confused

    # Combine all identified text columns with a colon
    new_index = df[label_cols].astype(str).agg(': '.join, axis=1).apply(clean_header_formatting)
    # Drop the text columns so they don't break the math
...
def process_ca(uploaded_file):
    try:
        ...
        # Apply Smart Indexer to merge Question IDs and Statements
        df = extract_smart_labels(df)
...
if st.session_state.processed:
    ...
    # PPTX Generation Button Interface
    with action_col2:
